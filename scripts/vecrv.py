...
def main():
    ...
    for i in range(0, len(mc_data), step):
        ...
        response = multicall.aggregate.call(mc_data[i:i+step], block_identifier=latest)[1]
        decoded = [vecrv.balanceOf.decode_output(data) for data in response]
        data['addresses'].update({replacements.get(addr, addr): balance for addr, balance in zip(lockers[i:i+step], decoded)})

    json.dump(data, datafile.open('w'))
    print(sum(data['addresses'].values()), vecrv.totalSupply[()](block_identifier=latest))
